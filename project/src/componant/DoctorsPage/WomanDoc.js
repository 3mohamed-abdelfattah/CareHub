...
import ChatbotWoman from "../ChatBOTS/ChatbotWoman";

const itemsPerPage = 5;

export default function WomanDoc() {
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/doctors/Gynecologist");
  ...
  const doctorElements = filteredDoctors.map((doctor, index) => (
    <div className="Doctors-D" key={index}>
      <div className="doctors-data-D">
        <h1>د.{doctor.name}</h1>
        <h3>تخـصص {doctor.specialization}</h3>
  ...
  return (
    <div>
      ...
      <h1 className="txtdoc">استشارات طبية دقيقة</h1>
      <h1 style={{ color: '#ff0505', textAlign: 'center' }}>...مع أفضل أطباء نسا وتوليد في بني سويف</h1>

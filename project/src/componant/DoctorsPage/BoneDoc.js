...
export default function BoneDoc() {
  const mystyle = {
    overflow: 'hidden',
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/doctors/orthopedist");
  ...
  return (
    ...
    <h1 className="txtdoc">الاستفادة من أحدث التقنيات</h1>
    <h1 style={{ color: '#ff0505', textAlign: 'center' }}>...مع أفضل أطباء العظام في بني سويف</h1>

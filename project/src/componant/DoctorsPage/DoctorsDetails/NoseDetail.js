...
export default function NoseDetail() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/doctors/Otorhinolaryngologist/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setDoctorData(data);
        console.log(data);
  ...
  return (
    <Fragment>
      <Header />
      <div style={body}>
        <h1 className="txtdoc">توفير بيئه امنه ومريحه</h1>
        <h1 style={{ color: "#ff0505", textAlign: "center" }}>
          احجز موعدًا مع افضل اطباء الانف والاذن والحنجره اليوم
        </h1>
        {loading ? (
          <div
            style={{

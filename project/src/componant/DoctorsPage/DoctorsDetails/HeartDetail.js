...
export default function HeartDetail() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/doctors/Cardiologist/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setDoctorData(data);
        console.log(data);
  ...
  return (
    <Fragment>
      <Header />
      <div style={body}>
        <h1 className="txtdoc">هل الم القلب يزعجك</h1>
        <h1 style={{ color: "#ff0505", textAlign: "center" }}>
          احجز موعدًا مع افضل اطباء القلب  اليوم
        </h1>
        {loading ? (
          <div
            style={{

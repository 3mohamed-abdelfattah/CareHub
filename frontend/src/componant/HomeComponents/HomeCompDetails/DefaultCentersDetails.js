...
export default function DefaultCentersDetails() {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`http://localhost:5000/api/c&mc/${id}`)
            .then((res) => res.json())
            .then((data) => {
                setDoctorData(data);
                console.log(data);
    ...
    const handleLinkedW = () => {
        ...
        if (phoneNumber) {
            ...
            setPlayStatue(Sound.status.PLAYING);
            toast.error("نأسف! لا تمتلك العياده رقم واتساب يمكنك الطلب من موقعنا وسيتم التواصل معك", options);
        }
    };
        ...
        <Fragment>
            ...
            <div style={body}>
                ...
                <h1 style={{ color: "#ff0505", textAlign: "center" }}>
                    نتطلع لخدمتك وتلبية احتياجاتك الصحية بأفضل الطرق الممكنة
                </h1>
                {loading ? (
                    <div
                        style={{
